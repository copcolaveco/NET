Public Class pSolicitud_autorizacion
    ...
    Public Function listarPorId(Optional solicitudId As Long = 0) As dSolicitud_Autorizacion
        Dim Solicitud_Autorizacion As New dSolicitud_Autorizacion
        Dim sql As String = "SELECT solicitudanalisis_id, usuario_autoriza_id, fecha, observaciones FROM solicitud_autorizacion WHERE "
        If solicitudId > 0 Then
            sql &= " solicitudanalisis_id = " & solicitudId
        End If
        ...
        Try
            ...
            If Not ds Is Nothing AndAlso ds.Tables.Count > 0 AndAlso ds.Tables(0).Rows.Count > 0 Then
                For Each f As DataRow In ds.Tables(0).Rows
                    Dim s As New dSolicitud_Autorizacion()
                    Solicitud_Autorizacion.SOLICITUDANALISIS_ID = CType(f.Item(0), Long)
                    Solicitud_Autorizacion.USUARIO_AUTORIZA_ID = CType(f.Item(1), Long)
                    Solicitud_Autorizacion.FECHA = CType(f.Item(2), String)
                    Solicitud_Autorizacion.OBSERVACIONES = CType(f.Item(3), String)
